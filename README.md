...
* `app/detection/yolo_detector.py` loads the supplied Ultralytics `yolov8n.pt` weights by default (they download on first use) and turns its boxes into `Detection` records. Set a class ID only when using custom weights; a custom one-class `bob` model is recommended because COCO's pretrained classes do not include pendulum bobs.
...
* `app/analysis/motion.py` converts pixels to pivot-relative coordinates (right/up convention), smooths only a copy using Savitzky–Golay, calculates `r` and unwrapped `theta`, estimates amplitude decay, fits the exponential spiral `r = A exp(-k theta)`, and compares it with constant, linear, and quadratic radial models using AICc.
* `app/visualization` produces separate x–time, y–time, x–y, r–theta, best-model, and spiral-fit charts. `app/export` writes CSV, JSON, PNG, and SVG assets.
...
The application selects the best-supported radial model by AICc, so it does not assume a pendulum follows a spiral; the exponential spiral is still reported and plotted for direct comparison. Fits with R² below 0.50 are labelled unreliable, and spiral fits additionally require non-negative decay. These are exploratory image-space models, not a physical proof of pendulum dynamics. Interpret damping and displacement in pixel units unless a camera calibration is supplied. Perspective, camera tilt, occlusions, motion blur, and low YOLO confidence can bias results.
